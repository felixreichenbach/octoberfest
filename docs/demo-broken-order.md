...
existing Synthetic Monitoring check (`oktoberfest-checkout-flow-minikube`,
check ID `1931`) go red, root-cause it via `gcx` (logs/traces) with Claude
Code, fix it, and redeploy straight to minikube.
...
```shell
gcx synthetic-monitoring checks status 1931
```
...
```shell
gcx synthetic-monitoring checks status 1931
gcx synthetic-monitoring checks timeline 1931 --from now-10m --to now
```
...
- `gcx synthetic-monitoring checks get 1931` — confirm which check/target.
